...
class StanfordTagger
{
    ...
    */
    ...
    private $jar = '/StanfordTagger/stanford-postagger-4.1.0.jar'; // the jar to use located in $path
    private $path = '/StanfordTagger/';	// path to where the standford postagger directory resides
    ...
    {
        return rtrim($this->path,'/').'/StanfordTagger/models/'.ltrim($this->model,'/');
    }
    ...
    public function tag($txt,$normalize = true,$separator = '')
    {
        ...
        {
            throw new CustomException("Jar not found: ".$this->get_jar());
        }
        if(!file_exists($this->get_model()))
        {
            throw new CustomException("Model not found: ".$this->get_model());
        }
        if($separator == '')
        {
        ...
        if(is_resource($process))
        {
            ...
            if($return_value == -1)
            {
                throw new CustomException("Java process error: ".$cmd);
            }
        }
